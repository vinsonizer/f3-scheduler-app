...
const AssignmentDetails = (props) => {
  ...
  const assignmentHandler = (target, index) => (event) => {
    console.log(
      "Got " + target + " for " + index + " with " + event.target.value
    );
    ...
    setAssignment({ ...assignment, assignmentStatus: newAssignment });
  };
  ...
  const submitHandler = (event) => {
    ...
    postApi(
      "/regions/" + regionId + "/aos/" + aoId + "/assignments",
      assignment,
      function (err, data) {
          ...
          <Row className="mb-3">
            <Form.Group as={Col} controlId="gridDate">
              ...
              <Form.Control
                ...
                value={dateStr}
                onChange={timestampHandler("timestamp")}
              />
            </Form.Group>
          </Row>
          <Row>
            {assignment.assignmentStatus.map((q, idx) => {
              return (
                <Form.Group key={idx} as={Col} controlId="gridQs">
                  <Form.Label>QIC</Form.Label>
                  <Form.Control
                    as="select"
                    aria-label="Q"
                    onChange={assignmentHandler("siteQId", idx)}
                  >
